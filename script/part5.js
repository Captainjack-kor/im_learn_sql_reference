...
const PART5_2_1 = `SELECT c.id, c.name FROM category AS c;`;
...
*/
const PART5_2_3 = `SELECT u.* FROM user AS u WHERE u.roleId IS NULL;`;
...
*/
const PART5_2_7 = `
    SELECT c.title, c.body, c.created_at
    FROM content AS c
    JOIN content_category AS cc 
...
*/
const PART5_2_8 = `
    SELECT c.title, c.body, c.created_at, u.name
    FROM content AS c
    JOIN content_category AS cc
    ON cc.contentId = c.id
    JOIN category AS cg
...
*/
const PART5_2_10 = `
    SELECT COUNT (c.id) AS 'ContentCount', u.name
    FROM user AS u
